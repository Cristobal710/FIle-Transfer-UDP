...
-- Crear el protocolo
local udp_file_transfer = Proto("udpft_custom", "UDP File Transfer Custom")

-- Definir los campos del protocolo para Stop-and-Wait
local fields = udp_file_transfer.fields
fields.seq_num = ProtoField.uint8("udpft_custom.seq_num", "Sequence Number", base.DEC, nil, 0x02)
fields.flag_end = ProtoField.uint8("udpft_custom.flag_end", "End Flag", base.DEC, {[0]="Data", [1]="End"}, 0x01)
fields.data_len = ProtoField.uint16("udpft_custom.data_len", "Data Length", base.DEC)
fields.data = ProtoField.bytes("udpft_custom.data", "Data")

-- Campos adicionales para Go-Back-N
fields.pkg_id = ProtoField.uint32("udpft_custom.pkg_id", "Package ID", base.DEC)

-- Campo para ACKs
fields.ack_num = ProtoField.uint8("udpft_custom.ack_num", "ACK Number", base.DEC)

-- Campo para identificar el tipo de protocolo
fields.protocol_type = ProtoField.string("udpft_custom.protocol_type", "Protocol Type")
...
local function is_file_transfer_packet(buffer, pinfo)
    ...
    -- También verificar si el contenido parece ser de nuestro protocolo
    if buffer:len() >= 1 then
        -- Verificar si contiene comandos conocidos
        local content = buffer():string()
        if content:match("UPLOAD") or content:match("DOWNLOAD") or 
           content:match("%.txt") or content:match("%.png") or content:match("%.jpg") then
            return true
        end
    end
