...
if [ $? -eq 0 ]; then
    ...
    echo "   - udpft_custom             (todos los paquetes del protocolo)"
    echo "   - udpft_custom.flag_end == 1      (solo paquetes END)"
    echo "   - udpft_custom.seq_num == 0       (solo paquetes con seq_num = 0)"
    echo "   - udpft_custom.ack_num            (solo paquetes ACK)"
