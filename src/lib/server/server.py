...
from lib.constants import (
    UPLOAD, DOWNLOAD, STOP_AND_WAIT, GO_BACK_N, ACK_TIMEOUT_SW,
    ACK_TIMEOUT_GBN, WINDOW_SIZE_GBN, WINDOW_SIZE_SW
)
from lib.protocol.protocol import (
    handshake_server, download_from_client, upload_from_client
)
...
def manage_client(
    channel: queue.Queue, addr, sock: socket, writing_queue,
    verbose=False, quiet=False
):
    # ✅ Pasar verbose y quiet
    conexion_type, protocol, name = handshake_server(
        channel, addr, writing_queue, verbose, quiet
    )
    ...
    if conexion_type == UPLOAD:
        ...
        if protocol == STOP_AND_WAIT:
            upload_from_client(
                name, channel, writing_queue, addr, STOP_AND_WAIT, sock
            )
        elif protocol == GO_BACK_N:
            upload_from_client(
                name, channel, writing_queue, addr, GO_BACK_N, sock
            )
        ...
        if protocol == STOP_AND_WAIT:
            download_from_client(
                name, writing_queue, addr, WINDOW_SIZE_SW, channel,
                ACK_TIMEOUT_SW
            )  # GBN con ventana de 1
        elif protocol == GO_BACK_N:
            download_from_client(
                name, writing_queue, addr, WINDOW_SIZE_GBN, channel,
                ACK_TIMEOUT_GBN
            )
...
class Server:
    ...
    def _start_writing_thread(self):
        self.writing_thread = threading.Thread(
            target=manage_writing, args=(self.writing_queue, self.sock)
        )
    ...
    def _listen(self):
        while True:
            try:
                ...
                # print(f">>> Server: recibí paquete de {len(pkg)} bytes "
                #       f"de {addr}")
                if addr in self.clients:
                    self.clients[addr][0].put(pkg)
                else:
                    # print(f">>> Server: Cliente nuevo {addr}, "
                    #       f"iniciando thread")
    ...
    def start_client(self, msg, addr, writing_queue):
        ...
        t = threading.Thread(
            ...
            args=(chan, addr, self.sock, writing_queue, self.verbose,
                  self.quiet)
        )
        self.clients[addr] = [chan, t]
        chan.put(msg)
        # print(f">>> Server: Thread iniciado para {addr}, "
        #       f"mensaje inicial: {len(msg)} bytes")
        t.start()
    ...
    def start_server(self, args):
        try:
            self.logger = setup_logging(
                'file_transfer_server', args.verbose, args.quiet
            )
