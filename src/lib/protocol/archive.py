...
class ArchiveSender:
    ...
    def __init__(self, path):
        ...
        self.archivo.seek(0)
        print(f"ArchiveSender inicializado: last_pkg_sent = "
              f"{self.last_pkg_sent}, posicion={self.archivo.tell()}")
    ...
    def next_pkg_go_back_n(self, seq_num=0):
        ...
        header += pkg_id
        pkg = header + data
        print(f"next_pkg_go_back_n: flag_end=0, seq_num={seq_num}, "
              f"pkg_id={int.from_bytes(pkg_id, 'big')}")
        return pkg, pkg_id
