...
def handshake_server(channel: queue.Queue, addr, writing_queue, verbose=False, quiet=False):
    ...
    try:
        ...
        logger.debug(f"Recibí conexion_type de {addr}")

        writing_queue.put(((0).to_bytes(4, "big"), addr))
        logger.debug(f"Envié ACK de conexion_type a {addr}")
        
        # Esperar protocol con timeout
        try:
            queue_size = channel.qsize()
            logger.debug(f"Cola de {addr} tiene {queue_size} paquetes antes de esperar protocol")
            protocol = channel.get(block=True, timeout=2.0)
            logger.debug(f"Recibí protocol de {addr}")
        except queue.Empty:
        ...
        while protocol == conexion_type:
            logger.debug(f"Cliente reenvió conexion_type, reenviando ACK a {addr}")
            writing_queue.put(((0).to_bytes(4, "big"), addr))
            try:
                protocol = channel.get(block=True, timeout=2.0)
            except queue.Empty:
        ...
        logger.debug(f"Envié ACK de protocol a {addr}")
        
        # Esperar name con timeout
        try:
            queue_size = channel.qsize()
            logger.debug(f"Cola de {addr} tiene {queue_size} paquetes antes de esperar name")
        ...
        # Reenviar ACK si el cliente reenvía el mismo paquete
        while name == protocol:
            logger.debug(f"Cliente reenvió protocol, reenviando ACK a {addr}")
            writing_queue.put(((1).to_bytes(4, "big"), addr))
            try:
                name = channel.get(block=True, timeout=2.0)
            except queue.Empty:
        ...
        logger.info(f"conexion_type={conexion_type}, protocol={protocol}, name={name}, addr={addr}")
    except Exception as e:
        logger.error(f"Error en manage_client: {e}")
        return
...
def download_from_client(name, writing_queue: queue.Queue, addr, window_sz, channel, timeout, verbose=False, quiet=False):
    ...
    if not os.path.exists(path):
        ...
        return
    logger.info(f"Archivo encontrado, empezando envío...")
    ...
    while not end:
        ...
        while(len(pkgs_not_ack) < window_sz and not file_finished):
            ...
            if pkg is None:
                ...
                first_byte = 1
                pkg = first_byte.to_bytes(1, "big") + (0).to_bytes(2, "big") + (pkg_id).to_bytes(4, "big")
                pkg_id_bytes = (pkg_id).to_bytes(4, "big")
                file_finished = True
                logger.debug(f"Creando paquete END con pkg_id={pkg_id}")
            writing_queue.put((pkg, addr))
            logger.debug(f"Envió paquete con flag_end={pkg[0] & 1}, pkg_id={pkg_id} (len={len(pkg)})")
        ...
        try:
            ...
            if len(pkg) == 4:
                ...
                logger.debug(f"ACK recibido para paquete {ack_num}")
                ...
                for pkg_id_bytes_key in pkgs_not_ack:
                    ...
                    if pkg_id_num <= ack_num:
                        ...
                        logger.debug(f"Removiendo paquete {pkg_id_num} de la ventana")
            ...
            if file_finished and not pkgs_not_ack:
                logger.info("Timeout pero no hay paquetes pendientes, finalizando")
                end = True
            else:
                logger.warning("Timeout, no recibí ACKs, reenvío los n paquetes")
                for value in pkgs_not_ack.values():
                    writing_queue.put((value, addr))
        except ConnectionResetError:
                logger.error("Conexión reseteada durante download")
                return
        except Exception as e:
                logger.error(f"Error durante download: {e}")
                return
...
def upload_from_client(name, channel, writing_queue: queue.Queue, addr, protocol=None, sock=None, verbose=False, quiet=False):
    ...
    logger.debug(f"upload_from_client iniciado para {name} desde {addr}")
    ...
    logger.debug(f"Esperando paquetes de {addr}, protocolo={protocol}")
    
    while not work_done:
        try:
            pkg = channel.get(block=True, timeout=30.0)
        except queue.Empty:
            logger.warning(f"Timeout esperando paquetes de {addr}")
            break
        
        flag_end, data_len, pkg_id, data = arch.recv_pckg_go_back_n(pkg)
        
        logger.debug(f"Recibí paquete flag_end={flag_end}, pkg_id={pkg_id}, esperado={expected_pkg_id}")
        ...
        if pkg_id == expected_pkg_id:
            ...
            if protocol == STOP_AND_WAIT:
                ...
                logger.debug(f">>> Server: SW - paquete fuera de orden pkg_id={pkg_id}, reenvío ACK{expected_pkg_id}")
            else:
                ...
                if expected_pkg_id > 0:
                    ...
                    logger.debug(f">>> Server: GBN - paquete fuera de orden pkg_id={pkg_id}, reenvío ACK{expected_pkg_id-1}")
                else:
                    # No enviamos ACK si aún no hemos recibido ningún paquete en orden
                    logger.debug(f">>> Server: GBN - paquete fuera de orden pkg_id={pkg_id}, no hay paquetes previos para ACK")
        ...
        if flag_end == 1:
            ...
            if protocol != STOP_AND_WAIT:
                ...
                logger.debug(f">>> Server: GBN - delay completado para envío final")
            
            logger.info(f">>> Server: Archivo {name} recibido completamente desde {addr}")
            work_done = True
            break
...
def handshake(sock: socket, name: str, type: str, protocol: str, server_addr, verbose=False, quiet=False):
    ...
    stop_and_wait(sock, type.encode(), 0, server_addr, verbose, quiet)
    stop_and_wait(sock, protocol.encode(), 1, server_addr, verbose, quiet)
    stop_and_wait(sock, name.encode(), 2, server_addr, verbose, quiet)
    
    logger.info("Handshake completado")
    
    # Delay para evitar que se mezclen paquetes
    time.sleep(1.0)
...
def stop_and_wait(sock: socket, msg, ack_number, addr, verbose=False, quiet=False):
    ...
    while not ack_recv and retry_count < max_retries:
        ...
        try:
            ...
            if recv_addr == addr and len(pkg) == 4 and (value == ack_number):
                ack_recv = True
            ...
            retry_count += 1
            logger.debug(f"Timeout, no recibí ACK (intento {retry_count}/{max_retries})")
            sock.sendto(msg, addr)
    
    if not ack_recv:
        logger.error(f"No se pudo completar el handshake después de {max_retries} intentos")
        raise Exception("Handshake failed")

def upload(sock: socket, arch: ArchiveSender, end, window_sz, server_addr, timeout, verbose=False, quiet=False):
    """
    Sube un archivo usando el protocolo Go Back N o Stop-and-Wait.
    """
    ...
    # Determinar si es Stop-and-Wait (ventana = 1)
    is_stop_and_wait = (window_sz == 1)

    logger.info(f"Iniciando upload {'SW' if is_stop_and_wait else 'GBN'} con ventana={window_sz}")
    ...
    while not end:
        ...
        while(len(pkgs_not_ack) < window_sz and not file_finished):
            ...
            # Obtener el paquete del archivo
            pkg, pkg_id_bytes = arch.next_pkg_go_back_n(pkg_id)
            ...
            if pkg is None:
                ...
                file_finished = True
                logger.debug(f">>> Cliente: Paquete END creado: pkg_id={pkg_id}")
            
            sock.sendto(pkg, server_addr)
            logger.debug(f">>> Cliente: Enviado paquete: pkg_id={pkg_id}, len={len(pkg)}, flag_end={pkg[0] & 1}")
        ...
        # Fase 2: Esperar ACKs
        logger.debug(f">>> Cliente: Esperando ACKs: {len(pkgs_not_ack)} paquetes sin confirmar")
            ...
            if file_finished and not pkgs_not_ack:
                ...
                for pkg_id_bytes_key, value in pkgs_not_ack.items():
                    ...
                    if retry_count >= max_retries_per_packet:
                        logger.warning(f">>> Cliente: Paquete {pkg_num} alcanzó máximo de reintentos ({max_retries_per_packet}), asumiendo entregado")
...
def download(sock: socket, arch: ArchiveRecv, server_addr, timeout, verbose=False, quiet=False):
    """Descarga un archivo usando Go Back N"""
    ...
    while not work_done:
        try:
            ...
            if len(pkg) < 7:
                logger.warning(f">>> Cliente: Paquete demasiado pequeño para el protocolo (len={len(pkg)}), ignorando...")
                continue
        ...
        if flag_end == 1:
            logger.info("Download completado (paquete END recibido)")
            work_done = True
        ...
        if pkg_id == expected_pkg_id:
            ...
            logger.debug(f"Paquete en orden: ACK{pkg_id} enviado")
            expected_pkg_id += 1
        else:
