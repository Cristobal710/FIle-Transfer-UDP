...
def handshake_server(
    channel: queue.Queue, addr, writing_queue, verbose=False, quiet=False
):
    ...
    try:
        ...
        try:
            protocol = channel.get(block=True, timeout=2.0)
        ...
        while protocol == conexion_type:
            logger.debug(
                f">>> Server: Cliente reenvió conexion_type, "
                f"reenviando ACK a {addr}"
            )
            writing_queue.put(((0).to_bytes(4, "big"), addr))
            try:
                protocol = channel.get(block=True, timeout=2.0)
        ...
        while name == protocol:
            logger.debug(
                f">>> Server: cliente reenvió protocol, "
                f"reenviando ACK a {addr}"
            )
            writing_queue.put(((1).to_bytes(4, "big"), addr))
            try:
...
def download_from_client(
    name, writing_queue: queue.Queue, addr, window_sz, channel, timeout,
    ...
    if not os.path.exists(path):
        ...
        return
    logger.info(
        ">>> Server: archivo encontrado, empezando envío con Go Back N..."
    )
    ...
    while not end:
        ...
        while (len(pkgs_not_ack) < window_sz and not file_finished):
            pkg, pkg_id_bytes = arch.next_pkg_go_back_n(pkg_id)
            if pkg is None:
                first_byte = 1
                pkg = (first_byte.to_bytes(1, "big") + (0).to_bytes(2, "big") +
                       (pkg_id).to_bytes(4, "big"))
                pkg_id_bytes = (pkg_id).to_bytes(4, "big")
                file_finished = True
                logger.debug(
                    f">>> Server: creando paquete END con pkg_id={pkg_id}"
                )
            writing_queue.put((pkg, addr))
            logger.info(
                f">>> Server: envió paquete con flag_end={pkg[0] & 1}, "
                f"pkg_id={pkg_id} (len={len(pkg)})"
            )
        ...
        # Fase 2: Esperar ACKs
        print(
            f">>> Server: termine de enviar los paquetes, "
            f"tengo que esperar ACK. Paquetes sin ACK: {len(pkgs_not_ack)}"
        )
        try:
            ...
            if len(pkg) == 4:
                ...
                logger.debug(
                    f">>> Server: ACK recibido para paquete {ack_num}"
                )
                ...
                for pkg_id_bytes_key in pkgs_not_ack:
                    ...
                    if pkg_id_num <= ack_num:
                        ...
                        logger.debug(
                            f">>> Server: removiendo paquete {pkg_id_num} "
                            f"de la ventana"
                        )
                for pkg_id_bytes_key in to_remove:
                    del pkgs_not_ack[pkg_id_bytes_key]
                # Si terminamos el archivo y no hay paquetes sin confirmar
                if file_finished and not pkgs_not_ack:
                    logger.debug(
                        ">>> Server: todos los paquetes confirmados, "
            ...
            if file_finished and not pkgs_not_ack:
                # Si terminamos el archivo y no hay paquetes sin confirmar
                logger.info(
                    ">>> Server: timeout pero no hay paquetes pendientes, "
                    "finalizando"
                )
                end = True
            else:
                logger.warning(
                    ">>> Server: timeout, no recibi ACKs, "
                    "reenvio los n paquetes"
                )
...
def upload_from_client(
    name, channel, writing_queue: queue.Queue, addr, protocol=None, sock=None,
    verbose=False, quiet=False
):
    """
    ...
    - GO_BACK_N: ACK del último paquete recibido en orden
    """
    logger = setup_logging('protocol.server.upload', verbose, quiet)
    logger.debug(
        f">>> Server: upload_from_client_go_back_n iniciado para {name} "
        f"desde {addr}"
    )
    # Usar path absoluto
    current_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    work_done = False
    logger.debug(
        f">>> Server: upload_from_client_go_back_n esperando paquetes "
        f"de {addr}"
    )
    while not work_done:
        try:
            pkg = channel.get(block=True, timeout=30.0)
        except queue.Empty:
        ...
        logger.debug(
            f">>> Server: recibí paquete flag_end={flag_end}, "
            f"pkg_id={pkg_id}, esperado={expected_pkg_id}"
        )
        if pkg_id == expected_pkg_id:
            if flag_end == 1:
                logger.debug(
                    f">>> Server: paquete final recibido (flag_end=1), "
                    f"pkg_id={pkg_id}"
                )
    ...
    arch.archivo.close()
    logger.info(
        f">>> Server: upload completado para {name} desde {addr}, "
        f"archivo cerrado"
    )
...
def handshake(
    sock: socket, name: str, type: str, protocol: str, server_addr,
    verbose=False, quiet=False
):
    """
    Realiza el handshake inicial con el servidor.
    Envía el tipo de conexión (UPLOAD o DOWNLOAD), el protocolo (SW o GBN)
    y el nombre del archivo.
    """
    logger = setup_logging('protocol.client.handshake', verbose, quiet)
    logger.info(
        f"Iniciando handshake: type={type}, protocol={protocol}, "
        f"name={name}"
    )
...
def stop_and_wait(sock: socket, msg, ack_number, addr, verbose=False,
                  quiet=False):
    """
    ...
    while not ack_recv and retry_count < max_retries:
        ...
        try:
            ...
            if (recv_addr == addr and len(pkg) == 4 and
                    (value == ack_number)):
                ack_recv = True
                ...
                logger.debug(
                    f"Paquete de dirección incorrecta: {recv_addr} "
                    f"(esperaba {addr})"
                )
            elif len(pkg) != 4:
                logger.debug(
                    f"Paquete no es ACK válido (len={len(pkg)}), "
                    f"ignorando..."
                )
        except socket.timeout:
            retry_count += 1
            logger.warning(
                f"timeout, no recibi ACK (intento {retry_count}/"
                f"{max_retries})"
            )
            sock.sendto(msg, addr)
    ...
    if not ack_recv:
        logger.error(
            f"Error: No se pudo completar el handshake después de "
            f"{max_retries} intentos"
        )
        raise Exception("Handshake failed")


def upload(sock: socket, arch: ArchiveSender, end, window_sz, server_addr,
           timeout, verbose=False, quiet=False):
    """
    Sube un archivo usando el protocolo Go Back N.
    Utiliza una ventana deslizante para enviar múltiples paquetes sin esperar
    confirmación.
    """
    ...
    while not end:
        logger.debug(
            f">>> Cliente: Ciclo principal - end={end}, "
            f"file_finished={file_finished}, "
            f"pkgs_not_ack={len(pkgs_not_ack)}"
        )
        # Fase 1: Enviar paquetes hasta llenar la ventana o terminar el archivo
        while (len(pkgs_not_ack) < window_sz and not file_finished):
            ...
            if pkg is None:
                first_byte = 1
                pkg = (
                    first_byte.to_bytes(1, "big") + (0).to_bytes(2, "big") +
                    (pkg_id).to_bytes(4, "big")
                )
                pkg_id_bytes = (pkg_id).to_bytes(4, "big")
                file_finished = True
                logger.debug(
                    f">>> Cliente: Creando paquete END con pkg_id={pkg_id}"
                )
            sock.sendto(pkg, server_addr)
            logger.debug(
                f">>> Cliente: Envié paquete pkg_id={pkg_id} "
                f"(len={len(pkg)})"
            )
        ...
        # Fase 2: Esperar ACKs
        logger.debug(
            f">>> Cliente: Terminé de enviar paquetes, esperando ACKs. "
            f"Paquetes sin ACK: {len(pkgs_not_ack)}"
        )
        ...
        while time.time() - start_time < timeout:
            ...
            if len(pkg) == 4:
                ack_num = int.from_bytes(pkg, "big")
                logger.debug(
                    f">>> Cliente: Procesando ACK {ack_num} (bytes: {pkg})"
                )
                if ack_num > last_ack:
                    to_remove = []
                    for pkg_id_bytes_key in pkgs_not_ack:
                        pkg_id_num = int.from_bytes(
                            pkg_id_bytes_key, 'big'
                        )
                        if pkg_id_num < ack_num:
                            to_remove.append(pkg_id_bytes_key)
                            logger.debug(
                    ...
                    logger.debug(
                        f">>> Cliente: Después del ACK - "
                        f"file_finished={file_finished}, "
                        f"pkgs_not_ack={len(pkgs_not_ack)}"
                    )
                    if file_finished and not pkgs_not_ack:
                        logger.info(
                            ">>> Cliente: Archivo terminado y todos los "
                            "paquetes confirmados, saliendo..."
                        )
                        end = True
                    break
                else:
                    logger.debug(
                        f">>> Cliente: ACK {ack_num} es menor que el último "
                        f"ACK {last_ack}, ignorando..."
                    )
                    continue
        ...
        if time.time() - start_time >= timeout:
            logger.warning(
                f">>> Cliente: Timeout esperando ACKs - "
                f"file_finished={file_finished}, "
                f"pkgs_not_ack={len(pkgs_not_ack)}"
            )
            ...
            if time.time() - transfer_start_time >= max_transfer_time:
                logger.error(
                    f">>> Cliente: TIMEOUT GLOBAL - Transfer excedió "
                    f"{max_transfer_time} segundos, abortando..."
                )
                return
            ...
            if file_finished and not pkgs_not_ack:
                # Si terminamos el archivo y no hay paquetes sin confirmar
                logger.info(
                    ">>> Cliente: Archivo terminado y no hay paquetes "
                    "sin confirmar, saliendo..."
                )
                end = True
            else:
                ...
                for pkg_id_bytes_key, value in pkgs_not_ack.items():
                    ...
                    if retry_count >= max_retries_per_packet:
                        logger.warning(
                            f">>> Cliente: Paquete {pkg_num} alcanzó máximo "
                            f"de reintentos ({max_retries_per_packet}), "
                            f"asumirndo entregado"
                        )
                ...
                if packets_to_retry:
                    logger.warning(
                        f">>> Cliente: Reenviando {len(packets_to_retry)} "
                        f"paquetes sin ACK"
                    )
                    for pkg_id_bytes_key, value, pkg_num in packets_to_retry:
                        sock.sendto(value, server_addr)
                        logger.debug(
                            f">>> Cliente: Reenvié paquete {pkg_num} "
                            f"(intento {pkg_retry_count[pkg_num]})"
                        )
                else:
                    logger.warning(
                        ">>> Cliente: Todos los paquetes alcanzaron el límite "
                        "de reintentos, asumiendo transferencia completa"
                    )
                    end = True
...
def download(sock: socket, arch: ArchiveRecv, server_addr, timeout,
    ...
    logger.info(
        ">>> Cliente: empezando a recibir archivo con Go Back N..."
    )
    expected_pkg_id = 3
    work_done = False
    ...
    while not work_done:
        try:
            logger.debug(
                f">>> Cliente: Esperando paquete del servidor "
                f"(expected_pkg_id={expected_pkg_id})..."
            )
            sock.settimeout(timeout)
            pkg, recv_addr = sock.recvfrom(1024)
            logger.debug(
                f">>> Cliente: Recibí paquete de {len(pkg)} bytes "
                f"de {recv_addr}"
            )
            ...
            if len(pkg) < 7:
                logger.warning(
                    ...
                    f"(len={len(pkg)}), ignorando..."
                )
                continue

        except socket.timeout:
            logger.debug(
                f">>> Cliente: Timeout esperando paquetes del servidor "
                f"(expected_pkg_id={expected_pkg_id})"
            )
            continue

        flag_end, data_len, pkg_id, data = arch.recv_pckg_go_back_n(
            pkg
        )
        logger.debug(
            f">>> Cliente: Procesando paquete - flag_end={flag_end}, "
            f"pkg_id={pkg_id}, expected_pkg_id={expected_pkg_id}, "
            f"data_len={data_len}"
        )
        ...
        if pkg_id == expected_pkg_id:
            if flag_end == 1:
                ...
                for i in range(1, 11):
                    sock.sendto(
                        pkg_id.to_bytes(4, "big"), server_addr
                    )
                logger.debug(
                    f">>> Cliente: Envié ACK final para paquete END {pkg_id}"
                )
                ...
                if (data_len != -1):
                    ...
                    logger.debug(
                        f">>> Cliente: Envié ACK{pkg_id} para paquete en orden"
                    )
                    expected_pkg_id += 1
        else:
            ...
            logger.debug(
                f">>> Cliente: Paquete fuera de orden (recibí {pkg_id}, "
                f"esperaba {expected_pkg_id}), reenvío ACK{last_ack}"
            )
